...
namespace Mnk.TBox.Plugins.Searcher.Code
{
    sealed class Worker : IDisposable
    {
        ...
        public Worker(AvailabilityChecker availabilityChecker, Action<Action> synchronizer, IPathResolver pathResolver, ImageSource icon)
        {
        ...
        private void ShowDialog(bool success)
        {
            if (success)
            {
                ...
                searchDialog.Value.ShowDialog(engine, pathResolver);
            }
        ...
        public void Fill(Config config)
        {
            ...
            engine = new SearchEngine(mainConfig.Index, pathResolver);
            UnloadIndexes();
        }
