...
namespace Mnk.Library.ParallelNUnit.Infrastructure.Packages
{
    public sealed class ProcessPackage : BasePackage
    {
        ...
        public override void DoRun(Action<IPackage> onReceive, IList<Result> allTests, IList<IList<Result>> packages, bool copyToSeparateFolders, string[] copyMasks, bool needSynchronizationForTests, int startDelay, Synchronizer synchronizer, IProgressStatus updater, bool needOutput)
        {
            try
            {
                testsRunner.Run(FilePath, allTests, packages, Server, copyToSeparateFolders, copyMasks, needSynchronizationForTests, DirToCloneTests, CommandToExecuteBeforeTests, startDelay, synchronizer, updater, needOutput, RuntimeFramework);
                onReceive(this);
            }
            catch (Exception ex)
            {
